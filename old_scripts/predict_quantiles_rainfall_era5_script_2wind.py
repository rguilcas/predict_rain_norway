...
angle = np.angle(ds_ml.data_in.sel(var_name='u850')+1j*ds_ml.data_in.sel(var_name='v850'), deg=True)
angle = xr.DataArray(angle, dims=ds_ml.data_in.sel(var_name='u850', drop=True).dims,coords=ds_ml.data_in.sel(var_name='u850', drop=True).coords)
module = np.sqrt(ds_ml.data_in.sel(var_name='u850')**2+ds_ml.data_in.sel(var_name='v850')**2)

ds_ml = xr.Dataset(dict(data_in = xr.Dataset(dict(angle=angle, module=module)).to_array('var_name').transpose('time','var_name','longitude','latitude'), data_out=ds_ml.data_out))
...
model = Wang2024(num_classes=10, num_channels_in=2, image_size=128*64, groups=1).to(device)
...
torch.save(model.state_dict(), 'trained_models/wang2024_2winds_mag_angle_ERA5_norwaeyW_precip_10quantiles.torch')
