...
def get_input_data_from_wandb_logger(wandb_logger,load=True):
    ...
    if season =='all':
        ...
        ds_out = ds_out.where(ds_out.time.dt.season==season, drop=True)
    ...
    ds_test = xr.Dataset(dict(inputs=ds_in.isel(time=test_indices), targets = ds_out.isel(time=test_indices)))
