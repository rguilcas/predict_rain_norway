...
y_train = torch.tensor(ds_train.data_out.values).type(torch.float32)
y_test = torch.tensor(ds_test.data_out.values).type(torch.float32)
...
model = Wang2024(num_classes=1, num_channels_in=1, image_size=128*64, groups=1).to(device)


# loss_fn = DistribLoss().to(device)
loss_fn = PinballLoss(0.7).to(device)
...
torch.save(model.state_dict(), 'trained_models/wang2024_ERA5_norwaeyW_precip_regression_z500.torch')
# Evaluate model
...
with torch.no_grad():
    ...
    pred_ds = xr.DataArray(pred, dims = ['time','var_name'], coords=ds_test.data_out.coords)
...
ds_bins = df_.groupby([pd.cut(df_.truth, np.arange(0,61,.5)),
             pd.cut(df_.pred, np.arange(0,61,.5)),
